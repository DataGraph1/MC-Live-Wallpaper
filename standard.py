...
def config():
    ...
    directory = configData['imagesPath']
    directory = f'{pathlib.Path().resolve()}\{directory}'
    timeslist = configData['timesList']
    ...
    timeSinceSync = 0
    currentFrame = 0
    weather = configData['weather']

    return directory, timeslist, sunRise_Start, day_Start, sunSet_Start, night_Start, framesForTime, backgroundUpdateTimer, now, month, currentSec, waitForNextFrame, timeSinceSync, currentFrame, weather

directory, timeslist, sunRise_Start, day_Start, sunSet_Start, night_Start, framesForTime, backgroundUpdateTimer, now, month, currentSec, waitForNextFrame, timeSinceSync, currentFrame, weather = config()
